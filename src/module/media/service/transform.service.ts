...
export class TransformService {
    ...
    async convertToHLS(inputPath: string, outputDir: string, videoHeight?: number): Promise<boolean> {
        ...
        const args = [
            ...
            "-hls_segment_filename", path.join(outputDir, "%v/seg%06d.ts"),
            ...
            ffmpeg.on("close", code => {
                if (code === 0) {
                    ...
                    ladder.forEach((r, i) => {
                        ...
                        masterContent = masterContent.replace(new RegExp(oldPathForward.replace(/\//g, '\\/'), "g"), newPath);
                        masterContent = masterContent.replace(new RegExp(oldPathBackward.replace(/\\/g, '\\\\'), "g"), newPath);
                    });
